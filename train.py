...
class Config(object):
    def __init__(self, args):
        ...
        elif args.scheduler in ['noam', 'cosine_annealing_warm']:
...
def run(args, config):
    ...
    for epoch in range(config.n_epochs):
        ...
        if args.model == 'transformer':
            ...
            valid_loss = transformer_eval_epoch(model, valid_dataloader, criterion, config.device)

        else:
            train_loss = seq2seq_train_epoch(model, train_dataloader, criterion, optimizer, config.clip, config.device)
            valid_loss = seq2seq_eval_epoch(model, valid_dataloader, criterion, config.device)
...
if __name__ == '__main__':
    ...
    assert args.data in ['wmt_sm', 'kor_sm']
    assert args.tokenizer in ['word', 'bpe', 'unigram']
    assert args.scheduler in ["constant", 'cosine_annealing_warm', 'noam', 'cosine_annealing', 'exponential', 'step']
